...
CanvasForge 提供两个互斥的 LLM 工具，由当前聊天 AI 按任务选择：
...
每条用户消息最多只能执行上述一个工具一次。任一工具返回失败后，当前 AI 必须直接说明原因并等待用户发送新消息，不得在两个工具之间切换、删除 `avatar_targets`、修改提示词或降级重试。插件也会在同一个消息事件上阻止第二次执行，避免模型反复调用或产生额外付费请求。
...
只有正式发布的 Release 才会被识别为可安装版本；当前代码版本为 `v0.1.3`。
