...
LLM Tool 会先完成模式、参考图、权限、配置、忙碌状态与冷却等预检。预检失败时，工具把明确原因交给当前聊天 AI，由 AI 告知用户后结束；预检成功时，受理结果会明确包含 `accepted=true`、`completed=false`，表示图片尚未生成或发送。当前聊天 AI 此时只能回复“正在生成，请稍等”，不能提前声称已经画好、完成或发送，也不应重复调用工具。插件会等这条等待回复发送并写入原会话后，才启动后台图像请求，避免失败消息与等待消息顺序颠倒。

成功后，插件先把图片发送到 QQ，再使用任务发起时的原会话、原人格和原聊天模型进行一次禁用工具的文本调用，让 AI 主动发送一句简短的完成通知，并把实际通知写回原会话历史。生成图片不会交给这次聊天模型查看，因此完成通知只确认图片已经发送，不描述模型未见过的画面细节。完成通知调用失败或返回空白时使用固定安全文案，不会重新生成图片。
...
聊天 AI 自己决定如何编写和扩展生图提示词。人物头像和回复图片会直接作为同一次 Images 编辑请求的参考图片上传，不会先调用视觉聊天模型进行看图或转述。图片成功发送后，CanvasForge 才额外调用一次禁用工具的聊天模型生成完成通知；工具调用本身不再接收预写完成语。旧的组合工具 `canvasforge_generate_image` 已删除。
...
选择头像后，聊天 AI 应在提示词中用“人物参考1、人物参考2……”分配人物、关系和位置。QQ 头像与直接回复图片中的所有可见人物都作为身份锚点：人物的脸部轮廓、稳定五官及比例，以及发型长度、刘海、分缝、卷直轮廓和发色默认以对应参考图为准。主提示词、人格记忆、昵称或模型先验中的冲突外貌描述不能覆盖参考图；只有用户在当前消息中明确要求改变某项身份特征时，才允许只覆盖该项，其余身份特征继续保持。存在这种例外时，聊天 AI 必须在提示词中明确说明该变更来自用户本轮要求，但不要求固定格式标记。

表情、视线、姿势、动作、服装、构图和场景由聊天 AI 根据任务自行决定并写入提示词，可以保留或调整参考图效果。多张参考图或单图多人时，人物不得融合、遗漏或互换；非人物参考内容不受人物身份规则约束。
...
当前代码版本为 `v0.1.9`。本版本在 `v0.1.8` 的全参考图人物锁定基础上重构异步流程：先发送并保存等待回复，再启动后台生图；图片成功发送后额外调用一次禁用工具的聊天 AI 生成完成通知并写入原会话历史；失败原因同样进入原会话并结束任务。既有 `v0.1.7`、`v0.1.8` Tag 与 Release 保持不变。

## 行为边界

- 全插件仅允许一个进行中的生成请求，不排队；
- 工具受理结果为 `accepted=true`、`completed=false`；原 AI 的等待回复发送并保存后才启动后台图像请求，命令则先发送固定等待语；
- 成功顺序固定为“等待回复 → 后台生成 → 图片 → 一次禁用工具的 AI 完成通知”，实际完成通知会写入任务发起时的原会话历史；
