...
LLM Tool 和 `/canvasforge` 命令完成基础校验并占用唯一任务位后会立即返回，生图在插件后台继续执行。LLM Tool 的受理结果会明确包含 `accepted=true`、`completed=false`，表示图片尚未生成或发送；当前聊天 AI 此时只能告知用户“正在生成，请稍等”，不能提前声称已经画好、完成或发送，也不应重复调用工具。

成功后，插件会先把图片直接发送到 QQ，再单独发送调用工具时由聊天 AI 预写的简短完成语；整个后台流程不会再次调用聊天模型。
...
- `canvasforge_text_to_image(prompt, completion_message)`：仅用于从零创作的纯文生图，可以按用户要求设计外貌；如果当前消息直接回复了图片，插件会在付费请求前拒绝，并明确要求改用图生图工具。
- `canvasforge_image_to_image(prompt, avatar_targets, completion_message)`：仅用于直接回复图片编辑或 QQ 人物头像参考；至少必须有一张回复图片或一个头像，没有参考图时会要求改用文生图工具。

两个工具的当前 Schema 都要求传入 `completion_message`。它由当前聊天 AI 按自身人格预写，最长 80 字，只能作为图片成功发送后的简短确认，不应描述 AI 尚未看见的生成细节。插件会把它规范为单行；旧调用缺少该字段、内容为空或类型错误时回退为“图片画好啦～”。聊天 AI 自己决定如何编写和扩展提示词；CanvasForge 不会额外调用一次聊天模型。人物头像和回复图片会直接作为同一次 Images 编辑请求的参考图片上传，不会先调用视觉聊天模型进行看图或转述。旧的组合工具 `canvasforge_generate_image` 已删除。
...
命令提示词会直接发送给图像接口，不经过聊天 AI 润色。`/canvasforge` 保持自动模式：当前消息直接回复了图片时执行图生图，否则执行文生图。命令成功生成并发送图片后使用固定完成语“图片画好啦～”。
...
当前代码版本为 `v0.1.8`。本版本强化所有参考图中人物的脸部、稳定五官、发型和发色约束，并补充异步受理状态与图片发送后的完成语；既有 `v0.1.7` Tag 与 Release 保持不变。

## 行为边界

- 全插件仅允许一个进行中的生成请求，不排队；
- 工具和命令在受理后立即返回；工具受理结果为 `accepted=true`、`completed=false`，后台完成后由插件直接发送图片或安全错误提示；
- 图片成功发送后才发送完成语；图片失败、任务取消或插件卸载取消时不发送完成语，且不会触发第二次聊天模型调用；
- 完成语发送失败只记录告警，不改变图片已经成功发送的结果或冷却状态；
